...
import { deleteSalonImage, listSalonImages } from "@/lib/salons.functions";
...
function GalleryPage() {
  ...
  const getImages = useServerFn(listSalonImages); const removeImage = useServerFn(deleteSalonImage);
  ...
  return <div className="mx-auto w-full max-w-6xl px-5 py-8"><SalonBranchTabs className="mb-7" /><div><h1 className="font-display text-3xl text-primary">Gallery</h1><p className="mt-1 text-sm text-muted-foreground">Photos for {activeSalon?.name ?? "the selected branch"}. Add or replace photos from the salon setup screen.</p></div>{!activeSalonId ? <div className="mt-8 rounded-2xl border border-dashed border-border py-16 text-center text-muted-foreground">Add a salon first to view its gallery.</div> : imagesQuery.isLoading ? <div className="flex justify-center py-20"><Loader2 className="size-5 animate-spin text-primary" /></div> : images.length === 0 ? <div className="mt-8 rounded-2xl border border-dashed border-border py-20 text-center"><Images className="mx-auto size-9 text-primary" /><p className="mt-3 font-medium">No photos for this branch yet</p><p className="mt-1 text-sm text-muted-foreground">Add location photos while editing this salon or branch.</p></div> : <><p className="mt-6 text-sm text-muted-foreground">{images.length}/10 photos</p><div className="mt-3 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{images.map((image) => <article key={image.id} className="group relative aspect-[4/3] overflow-hidden rounded-2xl border border-border bg-secondary"><img src={image.public_url} alt="Salon gallery" className="size-full object-cover" /><button aria-label="Remove photo" onClick={() => void remove(image)} className="absolute right-3 top-3 hidden rounded-full bg-card/95 p-2 text-destructive shadow-sm group-hover:block"><Trash2 className="size-4" /></button></article>)}</div></>}</div>;
}
